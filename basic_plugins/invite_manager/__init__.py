...
async def _(bot: Bot, event: FriendRequestEvent):
    if time_manager.add_user_request(event.user_id):
        ...
        if Config.get_config("invite_manager", "AUTO_ADD_FRIEND"):
            logger.debug("已开启好友请求自动同意，成功通过该请求", "好友请求", target=event.user_id)
...
async def _(bot: Bot, event: GroupRequestEvent):
    ...
    if str(event.user_id) in bot.config.superusers:
        try:
            logger.debug("超级用户自动同意加入群聊", "群聊请求", event.user_id, target=event.group_id)
    ...
    elif time_manager.add_group_request(event.user_id, event.group_id):
        logger.debug(
