...
class Mihoyobbs:
    ...
    async def init(self):
        ...
        # 如果这三个任务都做了就没必要获取帖子了
        if (
            not self.Task_do["bbs_Read_posts"]
            or not self.Task_do["bbs_Like_posts"]
            or not self.Task_do["bbs_Share"]
        ):
    ...
    async def get_tasks_list(self):
        ...
        if "err" in data["message"] or data["retcode"] == -100:
            ...
            elif data["data"]["states"][0]["mission_id"] >= 62:
    ...
    async def get_list(self) -> list:
        ...
        logger.info(f"已获取{len(temp_list)}个帖子")
        return temp_list
    ...
    async def read_posts(self):
        if self.Task_do["bbs_Read_posts"]:
            ...
            for i in range(self.Task_do["bbs_Read_posts_num"]):
                ...
                data = req.json()
                if data["message"] == "OK":
                    logger.debug(f"看帖：{self.postsList[i][1]} 成功")
                time.sleep(random.randint(2, 8))
    ...
    async def like_posts(self):
        if self.Task_do["bbs_Like_posts"]:
            ...
            for i in range(self.Task_do["bbs_Like_posts_num"]):
                ...
                data = req.json()
                if data["message"] == "OK":
                    logger.debug(f"点赞：{self.postsList[i][1]} 成功")
                # 判断取消点赞是否打开
    ...
    async def share_post(self):
        if self.Task_do["bbs_Share"]:
            ...
            for i in range(3):
                ...
                data = req.json()
                if data["message"] == "OK":
                    logger.debug(f"分享：{self.postsList[0][1]} 成功")
